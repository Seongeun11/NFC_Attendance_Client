...
class AttendanceController:
    ...
    def fetch_today_occurrences(self):
        """오늘 기준 회차 목록 및 이벤트 정보 조인 조회"""
        # UTC 대신 KST 기준으로 오늘 날짜 문자열을 만듭니다.
        today_str = datetime.now(KST).strftime("%Y-%m-%d")
        try:
            res = self.client.table("event_occurrences")\
                .select("*, events(*)")\
                .eq("occurrence_date", today_str)\
                .execute()
            items = res.data if res.data else []
            # 💡 [해결 핵심] DB에서 가져온 UTC 시간을 KST 문자열로 디코딩
            for item in items:
                if item.get("start_time"):
                    # "Z"를 "+00:00"으로 바꾸어 파싱한 뒤, .astimezone(KST)로 한국 시간 변경
                    utc_dt = datetime.fromisoformat(item["start_time"].replace("Z", "+00:00"))
                    kst_dt = utc_dt.astimezone(KST)
                    # 읽기 좋은 형태나 KST ISO 포맷으로 덮어쓰기
                    item["start_time"] = kst_dt.isoformat() 
