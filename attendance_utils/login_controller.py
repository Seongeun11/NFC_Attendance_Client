...
class LoginApp:
    def __init__(self, root, on_success=None): # 기본값 지정을 통해 매개변수가 안 들어와도 안전하게 방어
        ...
        self.client = SUPABASEAUTH  # 요청하신 SUPABASEAUTH 객체 바인딩
    ...
    def login(self, admin_id, pw):
        ...
        try:
            ...
            # 응답 객체 및 세션 유효성 검증
            is_success = result is not None and getattr(result, "session", None) is not None
            
            if is_success:
                print(f"[인증 성공] 관리자 이메일 세션 확보 완료: {email}")
