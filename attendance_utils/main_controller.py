...
class NfcApp:
    ...
    def show_main_frame(self):
        ...
        self.refresh_services_context()
        print("세션정보: ",self.client)
        print(f"[디버깅] 클라이언트 객체 정보: {self.client}")
        if self.client:
            try:
                # 간단한 더미 쿼리로 통신 가능 여부 테스트
                test = self.client.table("profiles").select("id").limit(1).execute()
                print("✅ Supabase 통신 성공!")
            except Exception as e:
                print(f"❌ Supabase 통신 실패: {e}")
                if self.client:
                    try:
                        # 로그인된 상태라면 세션 정보가 있어야 함
                        session = self.client.auth.get_session()
                        print(f"[디버깅] 현재 세션 정보: {session}")
                        if session:
                            print(f"[디버깅] 로그인 사용자 이메일: {session.user.email}")
                        else:
                            print("❌ 클라이언트 객체는 있으나, 인증된 세션(JWT)이 비어있습니다!")
                    except Exception as e:
                        print(f"세션 확인 중 에러: {e}")
            # 3. [핵심] 세션이 비어있는지 체크 후 강제 주입
        session = self.client.auth.get_session()
        if session is None:
            print("⚠️ 세션이 비어있음. 인증 토큰 재확인 필요.")
            # auth_manager가 세션을 제대로 로드하지 못했다면 명시적 호출
            # (AuthManager 구현에 따라 session_refresh 등을 호출해야 할 수도 있습니다)
        
        print(f"[디버깅] 최종 세션 유효성: {session is not None}")
        ...
        try:
           ...
           # 리스트박스 초기화 (즉시 실행)
            ...
            for u in users:
                ...
                # 🛠️ [무한루프 해결] 무의미한 after(0) 람다 스케줄러를 제거하고 즉시 화면에 삽입합니다.
    ...
    def _nfc_hardware_status_listener(self, status, target, uid_or_error):
        ...
        # 🚀 [오류 메시지 대시보드 출력 보완 및 버그 수정]
            ...
            def update_ui_error():
                # 🛠️ [해결안] 전역 Window 또는 렌더링된 자식 위젯 중에서 dashboard_ui 관련 알림 채널을 서칭합니다.
                notified = False
